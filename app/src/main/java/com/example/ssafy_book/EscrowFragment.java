...
public class EscrowFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        try {
            ...
            ownPrice.setText(resultEth);
            own = (Convert.fromWei(ethBalance.toString(),Convert.Unit.ETHER)).intValue();
            Log.d("이더 소유", resultEth);
        ...
        if(bundle != null){
            ...
            price.setText(bundle.getString("productPrice") + "Eth");
            pay = Integer.parseInt(bundle.getString("productPrice"));
            paymentPrice.setText("-" + bundle.getString("productPrice")+"Eth");
        ...
        Log.d("보유 이더 ",Integer.toString(own));
        Log.d("결제이더 ",Integer.toString(pay));
        int result = own - pay;
        Log.d("결제 후 보유 이더 ",Integer.toString(result));
        afterOwn.setText(Integer.toString(result) +"Eth");
        afterOwn.setTextColor(Color.BLUE);

        Bundle bundle2 = new Bundle(3);
        bundle2.putString("productName",pn);
        bundle2.putString("productPrice",Integer.toString(pay) + "Eth");
        bundle2.putString("afterOwn",Integer.toString(result) + "Eth");
                        ...
                        .setNegativeButton("네", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                ...
                                try {
                                    ...
                                    //contract주소 변경
                                    Escrow escrow = Escrow.load(contractId,web3j,credentials,new DefaultGasProvider());

                                    BigInteger amount = BigInteger.valueOf(pay).multiply(BigInteger.valueOf(10).pow(18));
                                    escrow.buyItem(amount).sendAsync().get();

                                    //상태 디비에 전송
                                    reqTask = new SendRequestTask();
                                    reqTask.execute("http://163.239.200.201:8080","api/board/update?boardid=" + boardId + "&state="+ 2, "GET");

                                    reqTask = new SendRequestTask();
                                    reqTask.execute("http://163.239.200.201:8080","api/board/update?boardid=" + boardId + "&buyer="+ accountId, "GET");
